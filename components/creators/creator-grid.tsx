...
import { ShoppingBag, Instagram, Youtube, BookText as TikTok } from "lucide-react";
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
      {creators.map((creator) => (
        <Card key={creator.id} className="overflow-hidden border-border/50 transition-all duration-300 hover:border-primary/50 hover:shadow-lg">
          <CardContent className="p-0">
            <div className="relative aspect-[16/7] bg-muted">
              <div className="absolute inset-0 bg-gradient-to-b from-black/60 to-transparent z-10" />
              <div className="absolute top-6 left-6 z-20 flex items-center space-x-3">
                <Badge variant="secondary" className="bg-background/80 backdrop-blur-sm text-sm py-1.5">
                  {creator.category}
                </Badge>
                {creator.verified && (
                  <Badge variant="outline" className="bg-primary text-primary-foreground border-none text-sm py-1.5">
                    Verified
                  </Badge>
                )}
              </div>
            ...
            <div className="pt-16 pb-8 px-8 text-center">
              ...
              <div className="flex justify-center space-x-8 mt-4">
                <div className="flex items-center text-muted-foreground hover:text-primary transition-colors">
                  ...
                  <span className="text-sm font-medium">{creator.socialsCount.instagram}</span>
                </div>
                <div className="flex items-center text-muted-foreground hover:text-primary transition-colors">
                  <Youtube className="h-5 w-5 mr-2" />
                  <span className="text-sm font-medium">{creator.socialsCount.youtube}</span>
                </div>
                <div className="flex items-center text-muted-foreground hover:text-primary transition-colors">
                  <TikTok className="h-5 w-5 mr-2" />
                  <span className="text-sm font-medium">{creator.socialsCount.tiktok}</span>
                </div>
              </div>
